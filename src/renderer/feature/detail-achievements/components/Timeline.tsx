...
const Timeline = () => {
  ...
  const processedData = useMemo(() => {
    ...
    const unlockedAchievements = game.achievements
  ...
  const stats = useMemo(() => {
    ...
    return { average, max, maxDate };
  }, [processedData]);

  return (
    <div>
      <div className="flex h-full w-full items-center justify-between gap-4">
        <StatsCard icon={Calendar} label="Daily Average" value={String(stats.average.toFixed(1))} />

        <StatsCard icon={TrendingUp} label="Best Day" value={String(stats.max)} />

        <StatsCard
          icon={Calendar}
          label="Peak Date"
          value={new Date(stats.maxDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
        />
      </div>
