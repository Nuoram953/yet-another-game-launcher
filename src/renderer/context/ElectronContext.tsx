...
const ElectronContext = createContext<GamesContextValue | undefined>(undefined);
...
export const GamesProvider: React.FC<GamesProviderProps> = ({ children }) => {
  ...
  const [filters, setFilters] = useState<FilterConfig>(
    JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEYS.FILTERS)) || { hasActiveFilters: false },
  );
  ...
  const updateFilters = useCallback((newFilters: Partial<FilterConfig>) => {
    ...
    setFilters((prev) => {
      ...
      localStorage.setItem(LOCAL_STORAGE_KEYS.FILTERS, JSON.stringify(cleanedFilters));
