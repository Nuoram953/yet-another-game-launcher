  ...
  "flex w-full rounded-md border px-3 py-2 text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2",
  {
    variants: {
      ...
      variant: {
        ...
        success: "border-green-500 bg-green-50 text-green-900 placeholder:text-green-300 focus:border-green-500 focus:ring-green-500",
      },
      // Size variants
      size: {
        sm: "h-8 text-xs",
...
// Export the type for the variants
  ...
  ({ 
    ...
    return (
      <div className="flex flex-col space-y-1.5">
        ...
        />
        {helperText && (
          <p className={`text-xs ${variant === 'error' ? 'text-red-500' : 'text-gray-500'}`}>
            {helperText}
          </p>
        )}
      </div>
