...
const inputStyles = cva(
  "flex w-full rounded-md px-3 py-2 text-design-inverted text-sm transition-colors focus:outline-none",
  {
    variants: {
      variant: {
      ...
      color: {
        dark: "!text-design-normal !bg-gray-700",
        light: "!text-design-inverted !bg-white",
      },
    },
    defaultVariants: {
      variant: "default",
    ...
    return (
      <div className="flex w-full flex-col space-y-1.5">
        {label && (
          <label
            htmlFor={props.id}
            className={`text-sm font-medium ${color == "dark" ? "text-design-normal" : "text-design-inverted"}`}
          >
            {label}
          </label>
        )}
        ...
        {helperText && (
          <p className={`text-xs ${variant === "error" ? "text-design-error" : "text-gray-500"}`}>{helperText}</p>
        )}
      </div>
    );
  },
