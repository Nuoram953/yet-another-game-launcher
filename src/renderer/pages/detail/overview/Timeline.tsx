...
import BadgeDropdown from "@render//components/dropdown/StatusSelection";
...
export const EventTimeline = () => {
  ...
  const getDateFromBigint = (timestamp: bigint): Date => {
    ...
    return timestampNumber > 10000000000 ? new Date(timestampNumber) : new Date(timestampNumber * 1000);
  };
  ...
  const sortedEvents = [...events].sort((a, b) => {
    const timestampA = typeof a.timestamp === "bigint" ? Number(a.timestamp) : (a.timestamp as Date).getTime();
    const timestampB = typeof b.timestamp === "bigint" ? Number(b.timestamp) : (b.timestamp as Date).getTime();
    return timestampA - timestampB;
  });

  const formatTime = (timestamp: bigint | Date): string => {
    const date = typeof timestamp === "bigint" ? getDateFromBigint(timestamp) : timestamp;
  ...
  return (
    <Card title={"Status History"} className="overflow-hidden">
      {sortedEvents.length > 0 ? (
          ...
          <div className="relative px-4">
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  className={`rounded-full p-1 ${visibleRange.start === 0 ? "bg-gray-800 text-gray-600" : "bg-gray-700 text-gray-300 hover:bg-gray-600"}`}
                >
                  <ChevronLeft size={16} />
                </button>
                <button
                  ...
                  className={`rounded-full p-1 ${visibleRange.end >= sortedEvents.length ? "bg-gray-800 text-gray-600" : "bg-gray-700 text-gray-300 hover:bg-gray-600"}`}
                >
                ...
                return (
                  ...
                  >
                    ...
                    {actualIndex < sortedEvents.length - 1 && (
                      <div
                        className="absolute left-6 top-1/2 h-full w-px bg-gray-700"
                    ...
                    <div className="relative flex items-start p-4">
                      {/* Icon Circle */}
                      <div
                        className={`relative mr-6 flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-gradient-to-br ${getEventColor(event.type)}`}
                      >
            ...
            <div className="mt-4 flex items-center justify-center">
              <div className="text-sm text-gray-500">
                Showing {visibleRange.start + 1}-{Math.min(visibleRange.end, sortedEvents.length)} of{" "}
                {sortedEvents.length} status updates
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-lg bg-gray-800/50 p-8 text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">Change the game status to start tracking progress</p>
        </div>
      )}
    </Card>
  );
