...
import { metadataManager, logger } from "../../../main/index";
...
export const downloadMedia = async (game: GameWithRelations, mediaType: MEDIA_TYPE, count: number, max: number) => {
  ...
  let path = await metadataManager.getImageDirectoryPath(mediaType, game);
  ...
  for (const image of data) {
    await metadataManager.downloadImage(mediaType, game, image.url, image.mime.split("image/")[1]);
  }
};
