...
export class ElectronLogger {
  ...
  private constructor(
    ...
    if (!this.config.filePath) {
      ...
      const logDir = this.isMain
        ? path.join(app.getPath("userData"), "logs")
        : path.join(os.tmpdir(), appName, "logs");
  ...
  private formatLogEntry(logEntry: LogEntry): string {
    const process = logEntry.rendererName
      ? `${logEntry.process}:${logEntry.rendererName}`
      : logEntry.process;
  ...
  private writeToFile(logEntry: LogEntry): void {
    ...
    try {
      if (fs.existsSync(this.logFilePath)) {
        ...
        if (
          stats.size >= (this.config.maxFileSize ?? DEFAULT_CONFIG.maxFileSize!)
        ) {
          this.rotateLogFiles();
        }
      }
