...
import { IMAGE_TYPE, RouteMedia } from "../../common/constant";
...
ipcMain.handle(RouteMedia.GET_ALL_MEDIA, async (_event, gameId) => {
  try {
    ...
    const icons = await MediaService.getMediaByType(IMAGE_TYPE.ICON, gameId);
    const logos = await MediaService.getMediaByType(IMAGE_TYPE.LOGO, gameId);
    const covers = await MediaService.getMediaByType(IMAGE_TYPE.COVER, gameId);
...
ipcMain.handle(RouteMedia.GET_LOGOS, async (_event, gameId, count) => {
  try {
    return MediaService.getMediaByType(IMAGE_TYPE.LOGO, gameId, count);
  } catch (e) {
    log.warn(ErrorMessage.ERROR_WHILE_FETCHING_MEDIA);
    log.debug(e);
    return [];
...
ipcMain.handle(RouteMedia.GET_ICONS, async (_event, gameId, count) => {
  try {
    return MediaService.getMediaByType(IMAGE_TYPE.ICON, gameId, count);
  } catch (e) {
    log.warn(ErrorMessage.ERROR_WHILE_FETCHING_MEDIA);
    log.debug(e);
    return [];
...
ipcMain.handle(RouteMedia.GET_COVERS, async (_event, gameId, count) => {
  try {
    return await MediaService.getMediaByType(IMAGE_TYPE.COVER, gameId, count);
  } catch (e) {
    log.warn(ErrorMessage.ERROR_WHILE_FETCHING_MEDIA);
    log.debug(e);
    return [];
...
ipcMain.handle(RouteMedia.GET_TRAILERS, async (_event, gameId, count) => {
  try {
    return await MediaService.getMediaByType(IMAGE_TYPE.TRAILER, gameId, count);
  } catch (e) {
    log.warn(ErrorMessage.ERROR_WHILE_FETCHING_MEDIA);
    log.debug(e);
    return [];
...
ipcMain.handle(RouteMedia.GET_ACHIEVEMENTS, async (_event, gameId) => {
  try {
    return await MediaService.getMediaByType(IMAGE_TYPE.ACHIEVEMENT, gameId);
  } catch (e) {
    log.warn(ErrorMessage.ERROR_WHILE_FETCHING_MEDIA);
    log.debug(e);
    return [];
